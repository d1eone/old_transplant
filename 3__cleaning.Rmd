...
CountNumbers.Strings <- function(x){
  length(unique(na.omit(as.numeric(unlist(strsplit(unlist(x), "[^0-9]+"))))))
}

Parse.Number.list <- function(x){
  unique(na.omit(as.numeric(unlist(strsplit(unlist(x), "[^0-9]+")))))
}
```
...
ILDco$SmokeAgeStop.ClinGen = ifelse(is.na(ILDco$age_stop_smoking),
                                    NA,
                                    ifelse(CountNumbers.Strings(ILDco$age_stop_smoking) >1, #if there are more than 1 numbers use 2nd
                                           Parse.Number.list(ILDco$age_stop_smoking)[2],
                                           Parse.Number.list(ILDco$age_stop_smoking)[1]
                                           )
                                    )
#ILDco$SmokeAgeStop.ClinGen[ILDco$MRN=="36141596"] = 
```
