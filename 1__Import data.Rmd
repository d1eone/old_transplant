...
h2. Load Libraries and set global constants
```{r Loading Libraries}
library(RCurl)
library(jsonlite)
```
...
txp_token= '733D673904E7A34A7BCCFE6B789AC276'
ClinGen_token= '988B6AE24F596A66026E82D50D0796FB'
PPMR_token= '5D09F30900B9FA4EFA667E684D22B8C1'
```

h2. Import JSON data from REDCap using API access
...
KEEP = c("PPMR_ID","TransplantID","MRN","TransplantStudy")
PPMR = PPMR[KEEP]
```
...
temp.txp$SGRQ_Status = temp.txp$ipf_questionnaire_complete
...
ALL_Txp$PGC_Status = ifelse(is.na(ALL_Txp$PGC_Status.x),
                            ifelse(is.na(ALL_Txp$PGC_Status.y),0,ALL_Txp$PGC_Status.y),
                            ALL_Txp$PGC_Status.x)
ALL_Txp$Herlihy_Status = ifelse(is.na(ALL_Txp$Herlihy_Status.x),
                        ifelse(is.na(ALL_Txp$Herlihy_Status.y),0,ALL_Txp$Herlihy_Status.y),
...
### How many Missing ... how many complete by Survey instrument?   
#### PGC
```{r}
