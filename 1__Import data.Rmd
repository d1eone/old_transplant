...
PPMR = rename(PPMR,TransplantID = log_transplant_id, PPMR_ID = subject_id, ConsentDATE = transp_cons_dat, DOB = dob_log, MRN = mrn_log)  
PPMR = select(PPMR, PPMR_ID,TransplantID, ConsentDATE, DOB, MRN)
...
```{r COPIED ILD API pull}
### ClinGen API pull of Probands ####
work1.ILD = postForm(
  uri=api_url,
  token= ClinGen_token,
  content='record',
...
### labels study arm as PROBAND or RELATIVES ####
work1df.ILD = fromJSON(work1.ILD)
work1df.ILD$arm = "proband"
work2df.ILD = fromJSON(work2.ILD)
work2df.ILD$arm = "relatives"
### COMBINES Probands and Relatives into one ILD df ####
work.ILD = rbind(work1df.ILD,work2df.ILD)
work.ILD$ClinicalGeneticsStudy = TRUE
work.ILD$LastName = toupper(work.ILD$demo_last)
work.ILD$FamilyNAME = toupper(work.ILD$familyname)
work.ILD$MRN = work.ILD$mrn
work.ILD$FamilyID = work.ILD$familyid
work.ILD$id[work.ILD$id == ""] = work.ILD$bwhild_id[work.ILD$id == ""] #replaces missing values of "id" using values from "bwhwork2.ILD_id"
work.ILD = work.ILD[,!(names(work.ILD) %in% c("mrn","familyid","familyname","demo_last"))] #drops original variables not capitalized

ILD_relatives = subset(work.ILD, work.ILD$arm == "relatives")
DF_FamilyIDs = unique(ILD_relatives[c("FamilyNAME","FamilyID")])
ILD_probands = subset(work.ILD,work.ILD$arm == "proband")
...
ILD = rbind(ILD_rel2,ILD_pro3)
ILD$id[ILD$id == "1006"] = "10006" #renames ID mismatch with Chanlab &PPMI
ILD$gender[ILD$gender == "0"] = "2" #Recodes female to character "2" to match PPMR
ILD$gender = factor(as.numeric(ILD$gender),labels = c("Male","Female"))
ILD$gender[ILD$gender == ""] = NA
ILD$ClinGen_ID = ILD$id
ILD = select(ILD, MRN, id, smoked_cigarettes, smoking_history, smoking_cigarettes_now, pack_day, yearstar, smoking_age_reg, yearstop, age_stop_smoking) #DROPS unwanted var
...
#     'fields[6]'='smoked_cigarettes',
#     'fields[7]'='packs_per_day',
#     'fields[8]'='smoking_cigarettes_now',
#     'fields[9]'='age_stop_smoking',
#     'fields[10]'='smoking_history',
#     'fields[11]'='pack_day',
#     'fields[12]'='yearstar',
#     'fields[13]'='yearstop',
#     'fields[14]'='demo_last',
...
# ILD = select(ILD, MRN, id, smoked_cigarettes, smoking_history, smoking_cigarettes_now, pack_day, yearstar, smoking_age_reg, yearstop, age_stop_smoking)
```
